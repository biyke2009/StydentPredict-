...
scaler=joblib.load('scarel (4).pkl')
...
async def predict(student:StudentSchema):
    ...
    parent1_0=[
        1 if new_parent == "bachelor' degree" else 0,
    ...
    features=list(student_dict.values()) + gender1or_0 + race1or_0 + parent1_0 + lunch1_0 + test0_1

    scaled_date=scaler.transform([features])
    print(model.predict(scaled_date))
    pred=model.predict(scaled_date)[0]
    return {'perdict': round (pred,2)}
